...
const EditFaculty = ({ isOpen, onClose, onSuccess, faculty }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await api.put(`/faculties/${faculty.id}`, formData);
            if (res.data.success) {
                onSuccess();
                onClose();
            }
        } catch (err) {
            setError(err.response?.data?.message || 'Có lỗi xảy ra khi cập nhật khoa');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-gray-100">
                ...
                <div className="px-6 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <h2 className="text-lg font-bold">Chỉnh Sửa Khoa / Viện</h2>
                    </div>
