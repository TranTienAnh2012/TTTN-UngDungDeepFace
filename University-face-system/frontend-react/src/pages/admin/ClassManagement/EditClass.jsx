...
const EditClass = ({ isOpen, onClose, onSuccess, academicClass }) => {
    ...
    useEffect(() => {
        if (academicClass) {
            setFormData({
                ...
                faculty_id: academicClass.faculty_id || '',
                academic_year: academicClass.academic_year || '2022-2026',
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await api.put(`/academic-classes/${academicClass.id}`, formData);
            if (res.data.success) {
                onSuccess();
                onClose();
            }
        } catch (err) {
            setError(err.response?.data?.message || 'Có lỗi xảy ra khi cập nhật lớp');
        } finally {
            setLoading(false);
        }
    };
