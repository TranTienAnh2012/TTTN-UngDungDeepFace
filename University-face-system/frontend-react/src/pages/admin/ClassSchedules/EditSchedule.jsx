...
const formatDT = (d) => {
    if (!d) return '';
    try {
        const date = new Date(typeof d === 'string' && !d.includes('T') ? d.replace(' ', 'T') : d);
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-gray-100">
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" required className="w-full p-2.5 border border-gray-300 rounded-xl bg-white text-gray-900 font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm shadow-sm" value={formData.room_name} onChange={e => setFormData({...formData, room_name: e.target.value})} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1">Bắt đầu *</label>
                            <input type="datetime-local" required className="w-full p-2.5 border border-gray-300 rounded-xl bg-white text-gray-900 font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm shadow-sm" value={formData.start_time} onChange={e => setFormData({...formData, start_time: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1">Kết thúc *</label>
                            <input type="datetime-local" required className="w-full p-2.5 border border-gray-300 rounded-xl bg-white text-gray-900 font-medium focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm shadow-sm" value={formData.end_time} onChange={e => setFormData({...formData, end_time: e.target.value})} />
                        </div>
                    </div>
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <button type="submit" disabled={isSubmitting} className="px-5 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold text-sm shadow-sm transition-colors">{isSubmitting ? 'Đang lưu...' : 'Lưu'}</button>
                    </div>
                </form>
            </div>
        </div>
