...
import { X, CalendarPlus, Clock, Building2, Sparkles } from 'lucide-react';
...
const CreateSchedule = ({ isOpen, onClose, onCreated }) => {
    ...
    const handleRoomChange = (e) => {
        ...
        if (!roomId) {
            setFormData(prev => ({ ...prev, room_id: '', room_name: '' }));
            return;
        }
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4 bg-white overflow-y-auto flex-1">
                    ...
                    <div className="p-3.5 bg-indigo-50/70 border border-indigo-100 rounded-xl space-y-2.5">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-indigo-900 flex items-center gap-1.5">
                                <Sparkles size={14} className="text-indigo-600" /> Chọn nhanh theo Ca học chuẩn:
                            </span>
                            <input
                                type="date"
                                value={selectedDate}
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        >
                            ...
                            {isSubmitting ? 'Đang lưu...' : 'Thêm Lịch Học'}
                        </button>
                    </div>
                </form>
            </div>
