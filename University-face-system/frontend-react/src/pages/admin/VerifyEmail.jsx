    ...
    useEffect(() => {
        ...
        const verify = async () => {
            try {
                ...
                setStatus('error');
                setMessage(error.response?.data?.message || 'Xác thực thất bại. Mã có thể đã hết hạn hoặc đã được xác thực trước đó.');
            }
        };
                ...
                <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 sm:p-10 text-center">
                    ...
                    {status === 'success' && (
                        <div className="flex flex-col items-center">
                            ...
                            <h2 className="text-xl font-bold text-gray-900">Xác thực thành công!</h2>
                            <p className="text-gray-500 mt-2">{message}</p>
                            <p className="text-xs text-indigo-600 font-medium mt-3 bg-indigo-50 px-3 py-1.5 rounded-full">
                                ⏳ Tự động chuyển đến trang Đăng nhập trong {countdown}s...
                            </p>
                            <Link 
                                to="/login" 
                                className="mt-6 w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3 rounded-xl transition-all block text-center"
                            >
                                Đăng nhập ngay
                            </Link>
                        </div>
